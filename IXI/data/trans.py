...
class RandomRotion(Base):
    ...
    def sample(self, *shape):
        self.axes_buffer = self.axes[np.random.choice(
            list(range(len(self.axes))))]  # choose the random direction
...
class CenterCropBySize(CenterCrop):
    def sample(self, *shape):
        # random crop [H,W,T] from img [240,240,155]
        ...
        self.buffer = [slice(None)] + [slice(s, s+i)
                                       for i, s in zip(size, start)]
        return size
...
class RandCrop3D(CenterCrop):
    def sample(self, *shape):  # shape : [240,240,155]
        ...
        self.buffer = [slice(None)] + [slice(s, s+k)
                                       for s, k in zip(start, size)]
        return size
...
class RandomIntensityChange(Base):
    ...
    def tf(self, img, k=0):
        ...
        scale_factor = np.random.uniform(1.0 - self.scale, 1.0 + self.scale, size=[
                                         1, img.shape[1], 1, 1, img.shape[4]])  # [0.9,1.1)
        ...
        for n in range(img.shape[0]):
            ...
            if sig > self.eps:
                if len(img.shape) == self.dim+2:
                    C = img.shape[-1]
                    for c in range(C):
                        img[n, ..., c] = ndimage.gaussian_filter(
                            img[n, ..., c], sig)
