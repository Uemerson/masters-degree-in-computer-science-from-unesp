...
parser.add_argument('--val-list', required=True, help='line-seperated list of validating files')
...
for epoch in range(args.initial_epoch, args.epochs):
    ...
    for val_step in range(validation_steps):
        ...
        val_loss_value = 0
        for n, loss_function in enumerate(losses):
            val_loss_value += loss_function(val_y_true[n], val_y_pred[n]) * weights[n]

        val_loss.append(val_loss_value.item())

    val_loss_mean = np.mean(val_loss)
    val_loss_info = 'val_loss: %.4e' % val_loss_mean
