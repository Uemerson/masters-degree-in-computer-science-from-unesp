...
def main():
    ...
    save_dir = "/home/hinton/uemerson/register/vxm_2_bpca_mse_1_diffusion_1/task_03/"
    model_idx = -1
    weights = [1, 1]
    ...
    with torch.no_grad():
        for data in file_names:
            ...
            #     np.float16
            # )
            np.savez(save_dir + "reg_{}.npz".format(file_name),
                     x_def.cpu().detach().numpy())
            # np.savez(save_dir + "disp_{}.npz".format(file_name), flow)
            np.savez(save_dir + "disp_{}.npz".format(file_name),
                     flow.detach().cpu().numpy().squeeze())
